...
python bin/pf.py platform-create --workplace ./workplace --id platform.example-app --title "Example Application Platform" --project-type example-app --knowledge-package docs.example-domain --apply
python bin/pf.py platform-contract-doctor --workplace ./workplace --platform platform.example-app
```

Create or register dependencies first, then create the platform contract. Use `--knowledge-package` and `--template` to link existing resource ids.
