...
> Optional example process from the `verification-workflow` domain pack.
...
Roles are declared in `examples/domain-packs/verification-workflow/processes/testing.yaml`; responsibility boundaries separate operator, primary agent, Director, Inspector, worker, and CLI tool duties.
