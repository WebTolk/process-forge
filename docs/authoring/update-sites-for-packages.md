...
Use `file:///` URLs for local mirrors and public smokes:
...
Tool updates must declare an update policy. `replace_file` is supported for
local-file smoke coverage. `custom_command_requires_confirmation` is documented
as blocked by default and must not be used to run arbitrary remote scripts.
