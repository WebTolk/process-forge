...
Значение `session_id` используется точно: регистр и знаки в нём значимы.
Имена файлов присутствия и переписки строятся по хешу этого значения. Старые
записи читаются по точному идентификатору в метаданных; новая запись присутствия
получает приоритет без дублирования сессии. Добавление сообщений сохраняет
доступ к старой переписке и не смешивает сессии, чьи прежние имена файлов совпали.
