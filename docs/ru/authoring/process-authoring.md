...
Process authoring в стандартном сценарии должен явно спросить решение верхнего
уровня по `evolve`. Если `evolve.enabled=true`, answers должны содержать
`mode`, `timing`, `default_scope`, `candidate_targets`, `extraction_hints`,
`privacy`, `apply_policy` и `required_outputs`. Если `evolve.enabled=false`,
нужна причина в `evolve.reason` или `evolve.decision.reason`.

Materializer переносит `evolve` из answers в
`processes/user/<process-id>.yaml`. Блок `evolve` не должен быть спрятан в
`metadata`, не должен быть software-only стадией, не описывает обучение LLM и не
изменяет global packages автоматически.
