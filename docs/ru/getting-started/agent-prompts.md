...
- Не выводите роли ProcessForge из текущей рабочей папки или имени каталога:
  `.codex`, `.claude`, `.agents` и любые пользовательские agent root не имеют
  встроенной семантики. Используйте явные пути для корня дистрибутива,
  workplace, глобального корня инструкций агента и корней проектов.
- Глобальный agent root можно зарегистрировать как knowledge root, но он не
  является ProcessForge-проектом, пока оператор явно не попросил подключить
...
- Полностью автоматический путь используйте только когда оператор явно просит
  автоматизацию. Начинайте с чтения доступных сведений об устройстве, объясните
  найденную структуру и предложенные сущности ProcessForge, затем применяйте
...
Фиксируйте размещение на машине как отдельные роли:

- `<processforge-root>`: установленный дистрибутив ProcessForge.
- `<workplace-path>`: состояние и реестры ProcessForge workplace.
- `<agent-root>`: необязательная глобальная папка конфигурации или инструкций агента.
- `<project-root>`: реальный проект, выбранный для `project-onboard`.

Если сессия агента запущена в `<agent-root>` или `<processforge-root>`,
считайте это только контекстом оператора. Не передавайте этот каталог как
`--project-root`, пока оператор явно не сказал, что именно он является целевым
проектом.

## Полностью автоматическая настройка workplace

Используйте этот путь только когда оператор явно попросил автоматическую
настройку. Автоматический режим всё равно остаётся предварительно согласуемым:
исследуйте устройство, объясните найденное, предложите сценарий, получите
подтверждение и только после этого применяйте изменения.

Начинайте с доступной только для чтения стадии `device-discovery` из
`workplace-initialization`. Изучите доступные `AGENTS.md`, навыки, локальную
документацию, платформы, цепочки инструментов, инструменты, настройки MCP и
корни проектов. Если у агента нет прав на широкий просмотр дисков, используйте
ограниченную область из процесса: глобальный `AGENTS.md` и настроенные корни
навыков, документации, платформ, цепочек инструментов и проектов.

После исследования подготовьте отчёт, кандидаты ресурсов и предложение
автоматической настройки. Переводите найденное в сущности ProcessForge до
применения: инструкции агента — в правила workplace, навыки — в процессы или
повторно используемые шаблоны, локальную документацию — в пакеты знаний или
ресурсы знаний, платформы — в платформенные контракты, цепочки инструментов и
инструменты — в контракты или реестры, MCP — в реестр MCP, корни проектов — в
кандидаты на подключение проекта.

Применяйте изменения только после подтверждения предложения автоматической
настройки.

Не превращайте сам глобальный agent root в проект только потому, что это
