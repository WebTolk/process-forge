...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="pf-builtin-evolve-") as raw:
        ...
        process = load_yaml(ROOT / "processes" / "core" / "process-authoring.yaml")
        process.pop("evolve", None)
        write_yaml(root / "processes" / "core" / "process-authoring.yaml", process)
        (root / "prompts").mkdir()
        (root / "prompts" / "process-authoring-agent.md").write_text("# Process Authoring Agent\n", encoding="utf-8")
        (root / "docs" / "processes").mkdir(parents=True)
        (root / "docs" / "processes" / "process-authoring.md").write_text("# Process Authoring\n", encoding="utf-8")
        (root / "examples" / "process-authoring" / "process-authoring").mkdir(parents=True)
        (root / "examples" / "process-authoring" / "process-authoring" / "README.md").write_text("# Process Authoring\n", encoding="utf-8")
