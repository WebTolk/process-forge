...
from process_execution_smoke_support import PROCESS, assignment, call_mcp, fixture, run, state, transition
...
def test_saved_gate_and_completion_freshness() -> None:
    with patch.dict(PROCESS, {"run_completion": {"gates": ["prepare-ready"]}}), fixture() as (workplace, project, started):
        ...
        gate_file.unlink()
        readiness = ProcessExecutionService(project, workplace, core).can_complete(run_id=started["run_id"])
        ...
        stopped = transition(workplace, project, "report", "verify-ready")
        require(stopped.get("action") == "incomplete", stopped)
...
def test_distinct_required_evidence_ids() -> None:
    stages = deepcopy(PROCESS["stages"])
    stages[0]["required_evidence"] = ["check-a", "check-b"]
    with patch.dict(PROCESS, {"stages": stages}), fixture() as (workplace, project, _started):
        result = call_mcp(workplace, "pf.work.transition", {
            "project_root": str(project), "outcome": "completed",
            "evidence": evidence(artifact("brief"), gate("prepare-ready"),
                {"kind": "evidence", "evidence_id": "check-a", "status": "passed"},
                {"kind": "evidence", "evidence_id": "check-b", "status": "passed"}),
        })
        require(result.get("next_stage_id") == "build", result)
...
def main() -> int:
    ...
    test_distinct_required_evidence_ids()
