...
```text
Подготовь ProcessForge на этой машине. Он находится в папке
<путь-к-processforge>.

Изучи README и используй docs/ru/getting-started/agent-prompts.md как командный
справочник.
...
Используй явные пути для дистрибутива ProcessForge, workplace, необязательного
глобального agent root и корней проектов. Не считай текущую папку проектом,
если я явно не выбрал её для project-onboard.
```
...
применяет изменения.
Папки конфигурации агентов вроде `.codex`, `.claude`, `.agents` или с любым
другим именем могут стать источниками инструкций или знаний; они не являются
проектами из-за имени или места запуска агента.
