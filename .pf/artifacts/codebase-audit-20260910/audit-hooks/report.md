...
Редакция приёмки первичным агентом. Оригинал shell-воркера сохранён в
`recovery-20260911/audit-hooks-original-attempt2.md`. Первичная независимая
проверка — `recovery-20260911/verify_hooks.py` и `hooks-primary-results.json`.
H-02/H-03/H-04 повторены на настоящих файловых данных/Core/ingress; H-01
не принят как подтверждённый дефект, поскольку его mock доказывает повторный
вызов, но не нарушение контракта доставки. Ниже сохранено рассуждение автора
с этим ограничением. H-02 проверяет живой PID и повреждённые ownership records;
два настоящих daemon одновременно не запускались.
...
|---|---|---|---|
| H-01 | Не назначен | Повторная доставка повторно dispatch-ит hooks | гипотеза, не принята primary |
...
## H-01 — повторный hook dispatch: непринятая гипотеза
...
startup = normalized_event({
    ...
    "source": "startup",
    "cwd": str(Path('fixture-project').resolve()),
